...
public class SelectionRegistration
{
     ...
     *
     * @param operation The IO operation code.
     ...
     * Constructor.
     *
     * @param interestOperations The IO operations interest.
     * @param selectionListener  The selection listener that will be notified.
     * @param wakeupListener     The wakeup listener that will be notified.
     */
    public SelectionRegistration(int interestOperations, SelectionListener selectionListener, WakeupListener wakeupListener)
    {
     ...
     * Constructor.
     *
     * @param selectableChannel  The parent selectable channel.
     * @param interestOperations The IO operations interest.
     * @param selectionListener  The selection listener that will be notified.
     * @param wakeupListener     The wakeup listener that will be notified.
     */
    public SelectionRegistration(SelectableChannel selectableChannel, int interestOperations, SelectionListener selectionListener, WakeupListener wakeupListener)
    {
        ...
        try
        {
            if (Context.getCurrentLogger().isLoggable(Level.FINEST))
            {
                Context.getCurrentLogger().log(Level.FINEST,
                        "Calling thread about to block on the NIO selection registration. Timeout: " + TimeUnit.MILLISECONDS.toMillis(IoUtils.TIMEOUT_MS) + " ms. Waiting: " + this.barrier.getNumberWaiting());
            }
     ...
     *
     * @param readyOperations The ready operations.
     * @throws IOException
     */
     ...
     *
     * @param canceling True if a canceling request is made.
     */
     ...
     * operations.
     *
     * @param interest The IO operations interest.
     ...
     *
     * @param readyOperations The IO operations ready.
     */
    ...
    public void unblock() throws IOException
    {
        ...
        {
            Context.getCurrentLogger().log(Level.FINEST,
                    "Calling thread about to unblock the NIO selection registration. Timeout: " + TimeUnit.MILLISECONDS.toMillis(IoUtils.TIMEOUT_MS) + " ms. Waiting: " + this.barrier.getNumberWaiting());
        }
        ...
        if (this.selectionKey.isValid())
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        Context.getCurrentLogger().log(Level.FINEST,
                                "Update key (old | new) : " + SelectionRegistration.getName(this.selectionKey.interestOps()) + " | " + SelectionRegistration.getName(
