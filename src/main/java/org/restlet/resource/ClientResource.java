...
public class ClientResource extends UniformResource
{
     ...
     *
     * @param metadata The metadata to accept.
     ...
     *
     * @param value The segment value to add.
     * @return The updated reference.
     ...
     * status is not returned, then a resource exception is thrown.
     *
     * @param <T>         The expected type for the response entity.
     * @param resultClass The expected class for the response entity object.
     ...
     * status is not returned, then a resource exception is thrown.
     *
     * @param mediaType The media type of the representation to retrieve.
     ...
     * thrown.
     *
     * @param relativeRef The URI reference of the child resource relatively to the
     ...
     * thrown.
     *
     * @param relativeUri The URI of the child resource relatively to the current
     ...
     *
     * @param method    The request method to use.
     * @param mediaType The preferred result media type.
     * @return The optional response entity.
     */
     ...
     *
     * @param method      The request method to use.
     * @param entity      The object entity to post.
     * @param resultClass The class of the response entity.
     ...
     *
     * @param method The request method to use.
     * @param entity The request entity to set.
     * @return The optional response entity.
     */
     ...
     * @param method     The request method to use.
     * @param entity     The request entity to set.
     * @param clientInfo The client preferences.
     * @return The optional response entity.
     */
     ...
     * @param references   The references that caused a redirection to prevent infinite
     *                     loops.
    ...
    protected void handle(Request request, Response response, List<Reference> references, int retryAttempt, Uniform next)
    {
        if (next != null)
        {
            ...
            if (isRequestEntityBuffering() && (entity != null) && (entity.isTransient() || (entity.getSize() == Representation.UNKNOWN_SIZE)) && entity.isAvailable())
            {
                request.setEntity(new org.restlet.engine.io.BufferingRepresentation(entity));
            }
            ...
            if (isResponseEntityBuffering() && (entity != null) && (entity.isTransient() || (entity.getSize() == Representation.UNKNOWN_SIZE)) && entity.isAvailable())
            {
                response.setEntity(new org.restlet.engine.io.BufferingRepresentation(entity));
            }
        }
        else
        {
     ...
     * {@link org.restlet.service.ConverterService}.
     *
     * @param entity The object entity to put.
     * @return The optional result entity.
     * @throws ResourceException
     * @see <a
     ...
     * exception is thrown.
     *
     * @param entity The request entity to store.
     * @return The optional result entity.
     * @throws ResourceException
     * @see <a
     * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.6">HTTP
     ...
     * @param next         The next handler handling the call.
     */
    protected void retry(Request request, Response response, List<Reference> references, int retryAttempt, Uniform next)
    {
        getLogger().log(Level.INFO,
                "A recoverable error was detected (" + response.getStatus().getCode() + "), attempting again in " + getRetryDelay() + " ms.");
     ...
     *
     * @param challengeResponse The authentication response sent by a client to an origin
     *                          server.
     ...
     * server.
     *
     * @param challengeResponse The proxy authentication response sent by a client to an
     *                          origin server.
     ...
     * @param secret     The user secret, such as a password or a secret key.
     */
    public void setProxyChallengeResponse(ChallengeScheme scheme, final String identifier, String secret)
    {
        setProxyChallengeResponse(new ChallengeResponse(scheme, identifier, secret));
    }

    /**
     * Sets the client-specific information.
     *
     * @param clientInfo The client-specific information.
     ...
     *
     * @param conditions The conditions applying to this request.
     ...
     *
     * @param entityBuffering True if transient entities should be buffered.
     ...
     *
     * @param hostRef The host reference.
     ...
     *
     * @param loggable True if the call is loggable
     */
     ...
     * followed for a single call.
     *
     * @param maxRedirects The maximum number of redirections that can be automatically
     *                     followed for a single call.
     */
     ...
     * null, then the associated request will be executed asynchronously.
     *
     * @param onResponseCallback The callback invoked on response reception.
     */
     ...
     *
     * @param originalRef The original reference.
     ...
     *
     * @param protocol The protocol used or to be used.
     */
     ...
     * prevent chunked encoding from being used an HTTP connector.
     *
     * @param requestEntityBuffering True if transient request entities should be buffered after
     *                               being received.
     */
     ...
     * retrieval.
     *
     * @param responseEntityBuffering True if transient response entities should be buffered after
     *                                being received.
     */
     ...
     *
     * @param retryAttempts The number of retry attempts before reporting an error.
     */
     ...
     *
     * @param retryDelay The delay in milliseconds between two retry attempts.
     */
     ...
     * Indicates if idempotent requests should be retried on error.
     *
     * @param retryOnError True if idempotent requests should be retried on error.
     */
    ...
    public <T> T wrap(Class<? extends T> resourceInterface)
    {
        ...
        result = (T) java.lang.reflect.Proxy.newProxyInstance(org.restlet.engine.Engine.getInstance().getClassLoader(),
                new Class<?>[] {ClientProxy.class, resourceInterface},
                h);
