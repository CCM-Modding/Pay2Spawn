...
public class LogService extends Service
{
    ...
    protected String getDefaultResponseLogMessage(Response response, int duration)
    {
        ...
        else
        {
            ...
            if (isIdentityCheck())
            {
                org.restlet.engine.log.IdentClient ic = new org.restlet.engine.log.IdentClient(request.getClientInfo().getUpstreamAddress(),
            ...
            if (!response.isEntityAvailable() || Status.REDIRECTION_NOT_MODIFIED.equals(response.getStatus()) || Status.SUCCESS_NO_CONTENT.equals(response.getStatus()) || Method.HEAD.equals(
                    request.getMethod()))
            {
                sb.append('0');
            }
    ...
    {
        return (getLoggableTemplate() == null) ? true : getLoggableTemplate().match(request.getResourceRef().getTargetRef().toString()) > 0;
    }
     ...
     * Indicates if the identity check (as specified by RFC1413) is enabled.
     *
     * @param identityCheck True if the identity check is enabled.
     */
     ...
     * the one of IIS 6.
     *
     * @param responseLogFormat The format to use when logging responses.
     * @see org.restlet.routing.Template for format syntax and variables.
     ...
     *
     * @param loggableTemplateRef The URI template of loggable resource references.
     ...
     * Sets the URI template of loggable resource references.
     *
     * @param loggableTemplate The URI template of loggable resource references.
     */
     ...
     *
     * @param name The name of the JDK's logger to use when logging access calls.
     */
     ...
     *
     * @param logPropertiesRef The URI reference of the log properties.
     */
     ...
     * Sets the URI reference of the log properties.
     *
     * @param logPropertiesUri The URI reference of the log properties.
     */
