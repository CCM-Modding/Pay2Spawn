...
public final class BioUtils
{
     ...
     *
     * @param file The individual file or directory to delete.
     ...
     *
     * @param file      The individual file or directory to delete.
     * @param recursive Indicates if directory with content should be deleted
     *                  recursively as well.
     ...
     * @param recursive      Indicates if directory with content should be deleted
     *                       recursively as well.
     * @param garbageCollect Indicates if the garbage collector should be run.
     * @return True if the deletion was successful or if the file or directory
     * didn't exist.
     */
     ...
     *
     * @param input The input stream to exhaust.
     ...
     *
     * @param representation The representation to evaluate.
    ...
    public static long getAvailableSize(Representation representation)
    {
        ...
        else if (representation.getRange().getSize() != Range.SIZE_MAX)
        {
            ...
            {
                return Math.min(representation.getRange().getIndex() + representation.getRange().getSize(),
                        representation.getSize()) - representation.getRange().getIndex();
            }
            else
            {
     ...
     * @param characterSet The character set. May be null.
     * @return The equivalent reader.
     * @throws UnsupportedEncodingException if a character set is given, but not supported
     */
     ...
     *
     * @param representation The representation to read from.
     * @return The character reader.
     * @throws IOException
     */
    public static Reader getReader(final org.restlet.representation.WriterRepresentation representation) throws IOException
    {
        Reader result = null;
     ...
     *
     * @param writer       The writer.
     * @param characterSet The character set used to write on the output stream.
     ...
     * @param characterSet The stream character set.
     ...
     * pipe stream.
     *
     * @param representation the representation to get the {@link java.io.OutputStream}
     *                       from.
     ...
     * @param characterSet The character set for encoding.
    ...
    {
        return toByteArray(chars, java.nio.charset.Charset.defaultCharset().name());
    }
    ...
    {
        return toCharArray(bytes, java.nio.charset.Charset.defaultCharset().name());
    }
     ...
     *
     * @param reader The characters reader.
     * @return The converted string.
     * @see InputStreamReader
     */
    public static String toString(Reader reader)
    {
        ...
        if (reader != null)
        {
            try
            {
                ...
                BufferedReader br = (reader instanceof BufferedReader) ? (BufferedReader) reader : new BufferedReader(reader, IoUtils.BUFFER_SIZE);
                char[] buffer = new char[2048];
