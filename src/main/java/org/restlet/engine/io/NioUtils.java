...
public class NioUtils
{
     ...
     * @param maxCopied    The maximum number of bytes copied by this call or 0 for
     *                     unlimited length.
    ...
    public static int copy(ByteBuffer sourceBuffer, ByteBuffer targetBuffer, long maxCopied)
    {
        ...
        int result = (maxCopied == 0) ? maxBuffer : Math.min((int) maxCopied, maxBuffer);
     ...
     *
     * @param fileChannel     The readable file channel.
    ...
    {
        return isBlocking(writableChannel) ? Channels.newOutputStream(writableChannel) : new NbChannelOutputStream(writableChannel);
    }
     ...
     *
     * @param selectableChannel The channel to monitor.
     * @param operations        The operations to be ready to do.
     * @throws IOException
     */
