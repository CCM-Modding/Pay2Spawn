...
public class HeaderUtils
{
     ...
     */
    private static final Set<String> STANDARD_HEADERS = Collections.unmodifiableSet(new CaseInsensitiveHashSet(Arrays.asList(HeaderConstants.HEADER_ACCEPT,
     ...
     */
    private static final Set<String> UNSUPPORTED_STANDARD_HEADERS = Collections.unmodifiableSet(new CaseInsensitiveHashSet(Arrays.asList(HeaderConstants.HEADER_PRAGMA,
     ...
     * {@link Series}.
     *
     * @param entity  The source entity {@link Representation}.
     * @param headers The target headers {@link Series}.
     */
     ...
     *
     * @param message The source {@link Message}.
     * @param headers The target headers {@link Series}.
     */
     ...
     * .
     *
     * @param request The {@link Request} to copy the headers from.
     * @param headers The {@link Series} to copy the headers to.
     */
     ...
     * {@link Series}.
     *
     * @param response The {@link Response} to copy the headers from.
     * @param headers  The {@link Series} to copy the headers to.
     */
    @SuppressWarnings("unchecked")
    public static void addResponseHeaders(Response response, Series<Header> headers)
    {
        ...
        if (response.getProxyChallengeRequests() != null)
        {
            for (ChallengeRequest challengeRequest : response.getProxyChallengeRequests())
            {
                addHeader(HeaderConstants.HEADER_PROXY_AUTHENTICATE,
                        org.restlet.engine.security.AuthenticatorUtils.formatRequest(challengeRequest, response, headers),
                        headers);
            }
        }
        ...
        if (response.getAuthenticationInfo() != null)
        {
            addHeader(HeaderConstants.HEADER_AUTHENTICATION_INFO,
                    org.restlet.engine.security.AuthenticatorUtils.formatAuthenticationInfo(response.getAuthenticationInfo()),
                    headers);
        }
    ...
    public static void copyExtensionHeaders(Series<Header> headers, Response response)
    {
        if (headers != null)
        {
            Series<Header> extensionHeaders = (Series<Header>) response.getAttributes().get(HeaderConstants.ATTRIBUTE_HEADERS);
            if (extensionHeaders == null)
            {
                extensionHeaders = new Series<Header>(Header.class);
                response.getAttributes().put(HeaderConstants.ATTRIBUTE_HEADERS, extensionHeaders);
            }
     ...
     *
     * @param headers        The headers to copy.
     * @param representation The representation to update or null.
