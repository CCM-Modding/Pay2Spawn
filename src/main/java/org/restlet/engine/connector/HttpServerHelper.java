...
public class HttpServerHelper extends ServerConnectionHelper
{
    ...
    protected boolean canHandle(Connection<Server> connection, Response response) throws IOException
    {
        ...
        if (nextResponse != null)
        {
            ...
            else
            {
                ...
                for (Iterator<Response> iterator = inboundWay.getMessages().iterator(); iterator.hasNext() && !found; )
                {
    ...
    {
        return new HttpInboundRequest(getContext(), connection, methodName, resourceUri, protocol);
    }
