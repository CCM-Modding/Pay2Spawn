...
public abstract class BaseHelper<T extends Connector> extends ConnectorHelper<T>
{
     ...
     * Constructor.
     *
     * @param connector  The helped connector.
     * @param clientSide True if it is helping a client connector.
     */
    ...
    protected ThreadPoolExecutor createWorkerService()
    {
        ...
        ThreadPoolExecutor result = new ThreadPoolExecutor(minThreads,
                ...
                queue,
                new LoggingThreadFactory(getLogger(), true));
    ...
    protected void execute(Runnable task)
    {
        try
        {
            if (!getController().isOverloaded() && (getWorkerService() != null) && !getWorkerService().isShutdown() && getController().isRunning())
            {
        ...
        {
            getLogger().log(Level.WARNING, "Unable to execute a " + (isClientSide() ? "client-side" : "server-side") + " controller task", e);
        }
    }
    ...
    {
        return Integer.parseInt(getHelpedParameters().getFirstValue("maxIoIdleTimeMs", "60000"));
    }
     ...
     *
     * @param response The message to handle.
     */
     ...
     *
     * @param response    The message to handle.
     * @param synchronous True if the current thread should be used.
     */
     ...
     *
     * @param status  The error status to set on the responses.
