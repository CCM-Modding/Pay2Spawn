...
public class HttpClientInboundWay extends ClientInboundWay
{
    ...
    {
        return (getMessageState() == MessageState.START) || ((getIoState() == IoState.IDLE) && (getMessageState() != MessageState.BODY) && !isEmpty());
    }
    ...
    public void onTimeOut()
    {
        for (Response rsp : getMessages())
        {
            ...
            {
                getMessages().remove(rsp);
                getHelper().onInboundError(Status.CONNECTOR_ERROR_COMMUNICATION, rsp);
            }
        }
