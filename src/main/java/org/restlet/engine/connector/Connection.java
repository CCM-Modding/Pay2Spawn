 ...
 *
 * @param <T> The parent connector type.
...
public class Connection<T extends Connector> implements SelectionListener
{
     ...
     * @param socketChannel The underlying NIO socket channel.
     * @param controller    The IO controller.
     * @param socketAddress The associated IP address.
     * @throws IOException
     */
     ...
     *
     * @param graceful Indicates if a graceful close should be attempted.
     */
    ...
    {
        return (getSocket() == null) ? null : (getSocket().getInetAddress() == null) ? null : getSocket().getInetAddress().getHostAddress();
    }
    ...
    {
        return (getSocketChannel() == null) ? null : getSocketChannel().socket();
    }
    ...
    {
        return (getMaxIoIdleTimeMs() > 0) && (System.currentTimeMillis() - getLastActivity()) >= getMaxIoIdleTimeMs();
    }
     ...
     *
     * @param socketChannel The underlying NIO socket channel.
     * @param controller    The underlying IO controller.
     * @param socketAddress The associated socket address.
     * @throws IOException
     */
     ...
     * Indicates if the connection should be persisted across calls.
     *
     * @param persistent True if the connection should be persisted across calls.
     */
     ...
     * Indicates if idempotent sequences of requests can be pipelined.
     *
     * @param pipelining True requests pipelining is enabled.
     */
     ...
     * and the way.
     *
     * @param registration The socket's NIO registration holding the link between the
     *                     channel and the way.
     */
    ...
    public void setState(ConnectionState state)
    {
        if (getState() != state)
        {
            if (getLogger().isLoggable(Level.FINER))
            {
                getLogger().log(Level.FINER, "Connection state (old | new) : " + this.state + " | " + state);
            }
