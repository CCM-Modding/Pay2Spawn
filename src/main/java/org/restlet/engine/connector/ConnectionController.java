...
public class ConnectionController extends Controller implements Runnable, WakeupListener
{
     ...
     * Constructor.
     *
     * @param helper The parent connector helper.
     */
     ...
     * operations.
     *
     * @param conn The connection to control.
     * @throws IOException
     */
    ...
    protected void doRelease()
    {
        ...
        {
            getHelper().getLogger().log(Level.WARNING, "Unable to close the NIO selector", e);
        }
    }
     ...
     *
     * @param selectedKey The selected key selected.
     * @throws IOException
     */
    protected void onSelected(SelectionKey selectedKey) throws IOException
    {
        ...
        try
        {
            if (getHelper().getLogger().isLoggable(Level.FINEST))
            {
                getHelper().getLogger().log(Level.FINEST, "NIO selection detected for key: " + selectedKey.attachment());
            }
        ...
        {
            getHelper().getLogger().log(Level.FINER, "Problem during NIO selection", cke);
            getNewRegistrations().add(((SelectionRegistration) selectedKey.attachment()));
        }
    }
     ...
     *
     * @param selectableChannel  The NIO selectable channel.
     * @param interestOperations The initial operations of interest.
    ...
    protected void registerKeys()
    {
        ...
        while (newRegistration != null)
        {
            if (getHelper().getLogger().isLoggable(Level.FINEST))
            {
                getHelper().getLogger().log(Level.FINEST, "Registering new NIO interest with selector: " + newRegistration);
            }
     ...
     *
     * @param sleepTime The max sleep time.
     * @throws IOException
    ...
    protected void selectKeys(long sleepTime) throws IOException, ClosedByInterruptException
    {
        ...
        if (getHelper().getLogger().isLoggable(Level.FINER))
        {
            getHelper().getLogger().log(Level.FINER,
                    "NIO controller about to sleep " + sleepTime + " ms, selecting among " + getSelector().keys().size() + " keys...\n");
        }
        ...
        if (selectCount > 0)
        {
            if (getHelper().getLogger().isLoggable(Level.FINER))
            {
                getHelper().getLogger().log(Level.FINER, "NIO controller selected " + selectCount + " key(s) !");
            }

            for (Iterator<SelectionKey> keys = getSelector().selectedKeys().iterator(); keys.hasNext(); )
            {
        ...
        {
            getHelper().getLogger().log(Level.FINER, "NIO controlled selected no key");
        }
    }
    ...
    public void wakeup()
    {
        if (getSelector() != null)
        {
            ...
            if (getHelper().getLogger().isLoggable(Level.FINER))
            {
                getHelper().getLogger().log(Level.FINER, "NIO controller woke up");
            }
        }
    }
