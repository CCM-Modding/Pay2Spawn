...
public class ServerConnectionController extends ConnectionController
{
     ...
     * Constructor.
     *
     * @param helper The target server helper.
     */
        ...
        else if (!isOverloaded())
        {
            try
            {
                ...
                if (socketChannel != null)
                {
                    ...
                    {
                        Connection<Server> connection = getHelper().checkout(socketChannel,
                                this,
                        ...
                        {
                            getHelper().getLogger().fine("Connection from \"" + connection.getSocketAddress() + "\" accepted. New count: " + getHelper().getConnections().size());
                        }
                    }
                    else
                    {
    ...
    protected void doInit()
    {
        ...
        try
        {
            getHelper().getServerSocketChannel().register(getSelector(), SelectionKey.OP_ACCEPT);
        }
        catch (IOException ioe)
        {
            getHelper().getLogger().log(Level.WARNING, "Unexpected error while registering an NIO selection key", ioe);
        }
