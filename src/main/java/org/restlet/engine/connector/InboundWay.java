...
public abstract class InboundWay extends Way
{
        ...
        if ((contentLength != Representation.UNKNOWN_SIZE && contentLength != 0) || chunkedEncoding || connectionClose)
        {
            ...
            if (chunkedEncoding)
            {
                ...
                inboundEntityChannel = new ReadableChunkedChannel(this,
                        ...
                        getConnection().getRegistration().getWakeupListener());
            }
            else
            {
    ...
    public int processIoBuffer() throws IOException
    {
        ...
        if ((getMessageState() == MessageState.BODY) && (getEntityRegistration() != null))
        {
            ...
            if (getIoState() == IoState.READY)
            {
                ...
                {
                    getEntityRegistration().getSelectionListener().onSelected(getEntityRegistration());
                }
            }
            else
            {
     ...
     *
     * @param builderIndex The line builder index.
     */
     ...
     *
     * @param entityRegistration The NIO selection registration of the entity.
     */
