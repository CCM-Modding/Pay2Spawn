...
public class ServerAdapter extends Adapter
{
    ...
    public void commit(HttpResponse response)
    {
        ...
        catch (Throwable t)
        {
            ...
            {
                getLogger().log(Level.INFO, "The connection was broken. It was probably closed by the client.", t);
            }
            else
            {
                getLogger().log(Level.SEVERE, "An exception occured writing the response entity", t);
                response.getHttpCall().setStatusCode(Status.SERVER_ERROR_INTERNAL.getCode());
