...
public abstract class Call
{
    ...
    public static boolean isBroken(Throwable exception)
    {
        ...
        {
            result = (exception.getMessage().indexOf("Broken pipe") != -1) || (exception.getMessage().equals(
                    "An existing connection must have been closed by the remote party.") || (exception.getMessage().equals(
     ...
     * Indicates if the confidentiality of the call is ensured (ex: via SSL).
     *
     * @param confidential True if the confidentiality of the call is ensured (ex: via
     *                     SSL).
     */
