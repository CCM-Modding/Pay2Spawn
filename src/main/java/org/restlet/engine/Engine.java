...
public class Engine
{
    ...
    public static final String DESCRIPTOR_AUTHENTICATOR_PATH = DESCRIPTOR + "/" + DESCRIPTOR_AUTHENTICATOR;

    public static final String DESCRIPTOR_CLIENT = "org.restlet.engine.ClientHelper";

    public static final String DESCRIPTOR_CLIENT_PATH = DESCRIPTOR + "/" + DESCRIPTOR_CLIENT;

    public static final String DESCRIPTOR_CONVERTER = "org.restlet.engine.converter.ConverterHelper";

    public static final String DESCRIPTOR_CONVERTER_PATH = DESCRIPTOR + "/" + DESCRIPTOR_CONVERTER;

    public static final String DESCRIPTOR_PROTOCOL = "org.restlet.engine.ProtocolHelper";

    public static final String DESCRIPTOR_PROTOCOL_PATH = DESCRIPTOR + "/" + DESCRIPTOR_PROTOCOL;

    public static final String DESCRIPTOR_SERVER = "org.restlet.engine.ServerHelper";

    public static final String DESCRIPTOR_SERVER_PATH = DESCRIPTOR + "/" + DESCRIPTOR_SERVER;
     ...
     * Complete version.
     */
    public static final String VERSION = MAJOR_NUMBER + '.' + MINOR_NUMBER + RELEASE_NUMBER;
    ...
    {
        return getInstance().getLoggerFacade().getLogger(clazz, defaultLoggerName);
    }
    ...
    {
        return getInstance().getLoggerFacade().getLogger(object, defaultLoggerName);
    }
    ...
    public ConnectorHelper<Client> createHelper(Client client, String helperClass)
    {
        ...
        if (client.getProtocols().size() > 0)
        {
            ...
            for (final Iterator<ConnectorHelper<Client>> iter = getRegisteredClients().iterator(); (result == null) && iter.hasNext(); )
            {
                connector = iter.next();
     ...
     */
    @SuppressWarnings("unchecked")
    public ConnectorHelper<org.restlet.Server> createHelper(org.restlet.Server server, String helperClass)
    {
        ...
        if (server.getProtocols().size() > 0)
        {
            ...
            for (final Iterator<ConnectorHelper<org.restlet.Server>> iter = getRegisteredServers().iterator(); (result == null) && iter.hasNext(); )
            {
                connector = iter.next();
     ...
     */
    public org.restlet.engine.security.AuthenticatorHelper findHelper(ChallengeScheme challengeScheme, boolean clientSide, boolean serverSide)
    {
    ...
    {
        getRegisteredConverters().add(new org.restlet.engine.converter.DefaultConverter());
    }
     ...
     * @param constructorClass The constructor parameter class to look for.
     */
    public void registerHelpers(ClassLoader classLoader, java.net.URL configUrl, List<?> helpers, Class<?> constructorClass)
    {
        try
        {
                    ...
                    protected java.net.URLConnection openConnection(java.net.URL url) throws IOException
                    {
                        return new java.net.URLConnection(url)
                        {
                            ...
                            public InputStream getInputStream() throws IOException
                            {
                                ...
                                if (context != null)
                                {
                                    Response response = context.getClientDispatcher().handle(new Request(Method.GET, this.url.toString()));
     ...
     *
     * @param loggerFacade The logger facade to use.
     */
     ...
     */
    public void setRegisteredAuthenticators(List<org.restlet.engine.security.AuthenticatorHelper> registeredAuthenticators)
    {
     ...
     */
    public void setRegisteredConverters(List<org.restlet.engine.converter.ConverterHelper> registeredConverters)
    {
     ...
     */
    public void setRegisteredServers(List<ConnectorHelper<org.restlet.Server>> registeredServers)
    {
